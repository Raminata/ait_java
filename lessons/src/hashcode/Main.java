...
equals via overrided equals true
       ...
       /*
        ...
        2.symmetric:    x.equals(y) возвращает тот же результат что и
        y.equals(х)

        3.transitive:   если x.equals(y) and y.equals(z) то x.equals(z)
        4.consistent:   значение equals  может меняться только если меняется
    какая-то из составлящих, содержащаяся в equals
        ...
        /*
        hashcode - это просто целое число и он есть   у каждого обьекта в
        Джава. Чтобы получить его значение
        */
        /*можно использовать метод hashcode().
        Т.к. этот метод является методом класса Object то он наследуется
        всеми классами в Джава.
        метод hashcode() должен возвращать одно и тоже значение хеш для двух
        обьектов, которые равны в соответствии с методом equals()*/

        /*правила для метода hashcode() ("контракт")
        1.internal consistency значение хешкода может меняться только если
        значения в equals  меняются
